...
if(empty($_SESSION["userag"]) and empty($_SESSION["passag"])){
	echo "<p><div align='center'>Sebelum membeli Tiket dari kami, Anda harus login terlebih dahulu</div><p>";
	...
	if($rp["stok"] == 0){
		echo "<div align='center'><p><b>STOK HABIS <br>untuk Tiket $rp[nama]</b></div><p>";
		}else{
			if($rowc == 0){
	mysqli_query($kon, "insert into cart (idproduk, idanggota, jumlahbeli, tglcart) values ('$_GET[idp]', '$_GET[idag]', 1, NOW())");
			...
			if($rcr["jumlahbeli"] >= $rp["stok"]){
				echo "<div align='center'><p><b>STOK TIDAK MENCUKUPI<br>untuk Produk $rp[nama]</b></div><p>";
			}else{
