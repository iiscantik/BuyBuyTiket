<button type="button" class="btn btn-dis">TIKET</button> &raquo;
<a href="<?php echo "?p=produkadd"; ?>"><button type="button" class="btn btn-add">Detail Tiket</button></a>
<br />
<?php
...
$rp = mysqli_fetch_array($sqlp);
	...
	if($rp["diskon"] > 0){
		...
		echo "<img src='../fotoproduk/$rp[foto1]' border='1' width='100px'>
			  ...
			  <b>$stok</b>
			  <hr>
			  <b>Posisi Nonton Konser : $rp[berat] </b>
			  <hr>
		...
		echo "<br><a href='?p=produkedit&id=$rp[idproduk]'><button type='button' class='btn btn-add'>Ubah Tiket</button></a>
			  <a href='?p=produkdel&id=$rp[idproduk]'><button type='button' class='btn btn-add'>Hapus Tiket</button></a>";
		echo "</div>";
		echo "</div><br>";
		echo "</div>";
?>