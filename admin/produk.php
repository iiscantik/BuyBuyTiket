<button type="button" class="btn btn-dis">Tiket Konser</button> &raquo;
<a href="<?php echo "?p=produkadd"; ?>"><button type="button" class="btn btn-add">Tambah Tiket</button></a>
<br />
<?php
$batas = 8;
$halaman = $_GET["pg"];
...
while($rp = mysqli_fetch_array($sqlp)){
	...
	if($rp["diskon"] > 0){
		...
		echo "<br><a href='?p=produkdetail&id=$rp[idproduk]'><button type='button' class='btn btn-add'>Detail</button></a>
			  <a href='?p=produkedit&id=$rp[idproduk]'><button type='button' class='btn btn-add'>Ubah Tiket</button></a>
